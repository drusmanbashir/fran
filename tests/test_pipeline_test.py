...
def test_apply_dataset_overrides():
    ...
    updated = pipeline.apply_dataset_overrides(
        conf, dataset="drli_short", fold=0, cache_rate=0.0
    )
...
def test_run_pipeline_preprocess_and_train_on_gpu1(monkeypatch):
    ...
    class FakeProject:
        ...
        def maybe_store_projectwide_properties(
            self, overwrite=False, multiprocess=False
        ):
    ...
    monkeypatch.setattr(
        pipeline,
        "confirm_plan_analyzed",
        lambda _p, _plan: {"src_fldr_full": False, "final_fldr_full": False},
    )
