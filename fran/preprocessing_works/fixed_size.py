...
from monai.transforms.utility.dictionary import DeleteItemsd, EnsureChannelFirstd, EnsureTyped, SqueezeDimd
import torch
...
class FixedSizeMaker(object):
    ...
    def process(self,dicis, spatial_size,output_folder_im,output_folder_lm,src_dest_labels=None):
        ...
        Rz= Resized(keys = ['image','lm'], spatial_size=spatial_size,mode= ["linear", "nearest"])
...
class FixedSizeDataGenerator(PatchDataGenerator):
    _default = "project"
    def __init__(self, project, data_folder, spatial_size,src_dest_labels=None) -> None:
    ...
    def create_tensors(self,num_processes=32):
        ...
        results = ray.get([
            c.process.remote(
                dicis, self.spatial_size,self.output_folder/("images"),self.output_folder/("lms"),src_dest_labels=self.src_dest_labels
    ...
    def output_folder(self):
        output_folder = folder_name_from_list(prefix="sze", parent_folder=self.fixed_size_folder, values_list=self.spatial_size)
...
# SECTION:-------------------- SETUP-------------------------------------------------------------------------------------- <CR>
    ...
    # TSL.labelshort
    data_folder= Path("/s/fran_storage/datasets/preprocessed/fixed_spacing/totalseg/spc_150_150_150")
    F = FixedSizeDataGenerator(project=P, data_folder=data_folder, spatial_size=96,src_dest_labels=src_dest_labels)
    F.setup(overwrite=False)
    F.process()
# %%
    output_folder = folder_name_from_list(prefix="sze", parent_folder=P.fixed_size_folder, values_list=[64,64,64])
    output_folder_im = output_folder/"images"
    output_folder_lm = output_folder/"lms"
...
# %%
    results = ray.get([
        c.process.remote(
            dicis, spatial_size,output_folder_im,output_folder_lm,src_dest_labels=src_dest_labels
        ) for c,dicis in zip(actors,dicis)
    ])
    # dici = L(dici)
# %%
