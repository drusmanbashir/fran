...
class HDF5ShardGenerator:
    ...
    def setup(self, case_ids=None, num_processes=8):
        ...
        if self.overwrite:
            ...
            existing_case_ids = {
                case_id
                for shard_meta in shard_manifest
        ...
        for case_id in requested_case_ids:
            ...
            total_bytes = (
                image_pt.stat().st_size
                + lm_pt.stat().st_size
                + indices_pt.stat().st_size
            )
            case_records.append(
                {
                    "case_id": case_id,
        ...
        shard_offset = (
            max(shard_meta["shard_idx"] for shard_meta in shard_manifest) + 1
            if shard_manifest
            else 0
        )
...
# %%
    ...
    hdf5_output_folder = Path(
        "/r/datasets/preprocessed/totalseg/fixed_spacing/spc_100_100_100_rsc6fdbff67/hdf5_shards/src_212_212_106"
    )
# %%  # T:block_start|FGBGIndicesResampleDataset.process
# SECTION:-------------------- process--------------------------------------------------------------------------------------  # T:block_meta|FGBGIndicesResampleDataset.process <CR>
    G = HDF5ShardGenerator(
        pt_folder=F.output_folder,  # T:self_ref|    pt_folder=self.output_folder,
        shard_folder=hdf5_output_folder,  # T:self_ref|    shard_folder=self.hdf5_output_folder,
...
# %%
    ...
    existing_img = {p.name for p in (G.output_folder / "images").glob("*.pt")}
    existing_lm = {p.name for p in (G.output_folder / "lms").glob("*.pt")}
    G.existing_pt_fnames = existing_img.intersection(existing_lm)
    print("Output folder: ", G.output_folder)
    print(
        "PT files fully processed in a previous session: ",
        len(G.existing_pt_fnames),
    )
    case_ids_done = [
        info_from_filename(fn, full_caseid=True)["case_id"]
        for fn in G.existing_pt_fnames
    ]
    G.df.loc[G.df["case_id"].isin(case_ids_done), "pt_processed"] = True
# %%
    images_folder = G.pt_folder / "images"
    lms_folder = G.pt_folder / "lms"
    indices_folder = G.pt_folder / "indices"

    for folder in (images_folder, lms_folder, indices_folder):
        if not folder.exists():
            raise FileNotFoundError(f"Required folder missing: {folder}")

    src_tag = "_".join(str(v) for v in G.src_dims)
    shards_folder = G.shard_folder / f"src_{src_tag}"
    manifest_fn = shards_folder / "manifest.json"
    maybe_makedirs([G.shard_folder, shards_folder])
    G.shards_folder = shards_folder
    G.manifest_fn = manifest_fn
    G.completed_shards = []
    G.pending_shards = []
    G.shard_jobs = []
    G.shard_paths = []

    manifest_fn = F.hdf5_manifest_fn
    F.existing_hdf5_fnames = set()
    committed_shards = 0
    # if manifest_fn.exists():
# %%
    import json
...
# %%
    F.existing_hdf5_fnames = {f"{case_id}.pt" for case_id in case_ids_done}
    print("Output folder: ", F.output_folder)
    print("HDF5 shards fully processed in a previous session: ", committed_shards)
    print(
        "HDF5 cases fully processed in a previous session: ",
        len(F.existing_hdf5_fnames),
    )
    case_ids_done = [
        info_from_filename(fn, full_caseid=True)["case_id"]
        for fn in F.existing_hdf5_fnames
    ]
# %%
...
#         manifest = json.loads(manifest_fn.read_text())
#         shard_manifest = manifest["shards"]
#         G._cleanup_uncommitted_shards(existing_shards, shard_manifest)
#         existing_shards = sorted(shards_folder.glob("shard_*.h5"))
#         existing_case_ids = {
#             case_id
#             for shard_meta in shard_manifest
...
#         if len(requested_case_ids) == 0:
#             print(f"No missing HDF5 shard cases requested for {shards_folder}")
#             G.completed_shards = list(shard_manifest)
#             G.shard_paths = sorted(shards_folder.glob("shard_*.h5"))
#             return G
#     if len(requested_case_ids) == 0:
#         raise ValueError(
#             f"No shared requested case IDs found across images/lms/indices in {G.pt_folder}"
#         )
#
# %%
#     case_records = []
...
#         )
#
#     shard_groups = G._build_shard_groups(case_records)
#     shard_offset = len(shard_manifest)
#     for shard_rel_idx, shard_cases in enumerate(shard_groups):
#         shard_idx = shard_offset + shard_rel_idx
#         shard_fn = shards_folder / f"shard_{shard_idx:04d}.h5"
#         G.shard_paths.append(shard_fn)
#         G.shard_jobs.append(
#             {
#                 "shard_fn": shard_fn,
#                 "shard_idx": shard_idx,
#                 "shard_cases": shard_cases,
#                 "src_dims": G.src_dims,
#                 "cases_per_shard": G.cases_per_shard,
#                 "compression": G.compression,
#                 "compression_opts": G.compression_opts,
#             }
#         )
#         G.pending_shards.append(
#             G._manifest_entry(
#                 shard_idx=shard_idx,
#                 shard_fn=shard_fn,
#                 shard_cases=shard_cases,
...
#     G.completed_shards = list(shard_manifest)
#     G._persist_manifest(
#         shard_manifest=G.completed_shards,
#         pending_shards=G.pending_shards,
#     )
#
# %%
#
