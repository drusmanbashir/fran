...
class SingleCaseAnalyzer:
    ...
    def __init__(self, case_files_tuple, percentile_range: list, bg_label=0):
        """
        ...
        for k in self.img.meta.keys():
            if k not in excluded:
                ...
                if isinstance(v1, np.ndarray):
                    assert is_close(v1, v2, eps=1e-4), (
                        "Metadata mismatch for key: {0} . Filename: {1}".format(
                            k, self.mask.meta["filename_or_obj"]
                        )
                    )
...
class MultiCaseAnalyzer(GetAttr):
    ...
    def filter_unprocessed_cases(self):
        ...
        headline(
            "Found {0} new cases\nCases already processed in a previous session: {1}".format(
        ...
        if len(new_cases) == 0:
            ...
            self.new_cases = [
                [c["img_symlink"], c["lm_symlink"]]
                for c in all_cases
                if c["case_id"] in new_cases
            ]
        ...
        with h5py.File(self.h5f_fname, mode) as h5f:
            for output in self.outputs:
                try:
                    ...
                    print(
                        "Case id {} already exists in h5f file. Skipping".format(
                            output["case"]["case_id"]
                        )
                    )
...
if __name__ == "__main__":
    ...
    fn = Path(fn)
    dd = load_dict(fn)
    bboxes_info = load_dict(
        "/s/fran_storage/datasets/preprocessed/fixed_spacings/litsmc/spc_080_080_150/bboxes_info_bk.pkl"
    )
            ...
            stats.update(
                get_label_stats(
                    ...
                    dusting_threshold=int(
                        label_info["dusting_threshold"] * A.dusting_threshold_factor
                    ),
                )
            )
    ...
    args_list = [
        [case_tuple, M.bg_label, get_voxels] for case_tuple in M.list_of_raw_cases
    ]
    # %%
