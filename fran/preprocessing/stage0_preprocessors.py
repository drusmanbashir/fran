...
class PermuteSITKToTorch(ItemTransform):
    """
...
def niipair_to_torch_wrapper(
    ...
    overwrite,
        ...
        N = NiipairToTorch(
            output_img_folder, output_mask_folder, global_properties, spacings
        )
    ...
    def process_sitk_to_tensors(self, single_case_properties, overwrite=True):
        ...
        if self.proceed(overwrite) == True:
            ...
            pipeline2 = Pipeline(
                [
                    SITKToNumpy,
                    NumpyToTorch(torch.float16 if self.half_precision else torch.float32,torch.uint8),
                    PermuteSITKToTorch,  # ALWAYS DO THIS BEFORE RESIZING !!
                    Unsqueeze,
                    Unsqueeze,
...
if __name__ == "__main__":
    ...
    output_folder = Path("/media/ub/UB11/datasets/tmp/")
