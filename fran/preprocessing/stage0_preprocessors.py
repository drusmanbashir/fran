...
from fran.data.dataset import NormaliseClipd, ResamplerDataset
...
def crop_clip(case, proj_defaults):
    ...
    img_gantry_cropped[img_gantry_cropped < proj_defaults.HU_clip_range[0]] = (
        proj_defaults.HU_clip_range[0]
    )
    img_gantry_cropped[img_gantry_cropped > proj_defaults.HU_clip_range[1]] = (
...
class ResampleDatasetniftiToTorch(GetAttr):
    ...
    def resampling_output_folder(self, value):
        ...
        self._resampling_output_folder = folder_name_from_list(
            prefix="spc",
            parent_folder=self.fixed_spacings_folder,
            values_list=value,
        )
        print(
...
class GetSizeDest(ItemTransform):
    ...
    def encodes(self, x):
        sz_dests = [
            get_sitk_target_size_from_spacings(x_, self.spacings) for x_ in list(x)
        ]
        if test_eq(*sz_dests):
            print(
                "Differing sized arrays: {0} and {1}".format(
...
class NumpyToTorch(ItemTransform):
    def __init__(self, img_dtype=torch.float32, label_dtype=torch.uint8):
        store_attr()
...
class GetFilenames(ItemTransform):
    def encodes(self, single_case_properties):
        img_fname = single_case_properties["properties"]["img_fname"]
        lm_fname = single_case_properties["properties"]["lm_fname"]
        return img_fname, lm_fname
...
class NiipairToTorch(DictToAttr):
    ...
    def process_sitk_to_tensors(self, single_case_properties, overwrite=True):
        ...
        if self.proceed(overwrite) == True:
            ...
            pipeline1 = Pipeline(
                [GetFilenames, ReadSITKImgMask, GetSizeDest(self.spacings)]
            )
            ...
            if self.half_precision == True:
                pipeline2.append(HalfPrecision)
    ...
    def _create_output_filenames(self, single_case_properties):
        img_fn = Path(single_case_properties["properties"]["img_fname"])
        fn = strip_extension(img_fn.name)
        self.output_filenames = [
            output_folder / (fn + ".pt")
            for output_folder in [self.output_img_folder, self.output_lm_folder]
        ]
...
if __name__ == "__main__":
    globalp = load_dict(Path("/s/fran_storage/projects/lilun3/global_properties.json"))
    case_props = load_dict(
        Path("/s/fran_storage/projects/lilun3/raw_dataset_properties.pkl")
    )
# %%
...
# %%
    ...
    N.L = LoadSITKd(keys=['image'], image_only=True,ensure_channel_first=False,simple_keys=True)
    N.E = EnsureChannelFirstd(keys=["image"], channel_dim="no_channel")
    N.S = Spacingd(keys=["image"], pixdim=N.dataset_params['spacings'])
    N.N = NormaliseClipd(
        keys=["image"],
        clip_range=N.dataset_params["intensity_clip_range"],
        mean=N.dataset_params["mean_fg"],
        std=N.dataset_params["std_fg"],
    )
...
# %%
    ...
    fldr = Path(
        "/s/fran_storage/datasets/preprocessed/fixed_spacings/nodes/spc_078_078_375/masks/"
    )
