...
class LBDSamplerWorkerImpl(RayWorkerBase):
    def create_transforms(self, device="cpu"):
        from monai.transforms import Compose, CropForegroundd, EnsureChannelFirstd, ToDeviced
        try:
            from monai.transforms import MapLabelValued
        except ImportError:
            from monai.transforms import MapLabelValueD as MapLabelValued

        # adjust to your project
        from fran.transforms.io import LoadTorchd
        from fran.transforms.indices import FgBgToIndicesd2
        from fran.transforms.misc import DummyTransform

        self.image_key = "image"
        self.lm_key = "lm"
        tnsr_keys = [self.image_key, self.lm_key]

        margin = [int(self.plan["expand_by"] / sp) for sp in self.plan["spacing"]] if self.plan.get("expand_by") else 0

        LT  = LoadTorchd(keys=tnsr_keys)
        E   = EnsureChannelFirstd(keys=tnsr_keys, channel_dim="no_channel")
        C   = CropForegroundd(keys=tnsr_keys, source_key=self.lm_key, allow_smaller=True, margin=margin)

        rl = self.plan.get("remapping_lbd")
        R = (MapLabelValued(keys=[self.lm_key], orig_labels=rl[0], target_labels=rl[1])
             if (rl is not None and len(rl) == 2) else DummyTransform(keys=[self.lm_key]))

        Ind = FgBgToIndicesd2(keys=[self.lm_key], image_key=self.image_key,
                               ignore_labels=self.plan.get("fg_indices_exclude", []),
                               image_threshold=-2600)
        D   = ToDeviced(device=device, keys=tnsr_keys)

        self.transforms = Compose([LT, E, C, R, Ind, D])

    def _process_row(self, row: pd.Series) -> Dict[str, Any]:
        data = {"image": row["image"], "lm": row["lm"]}
        data = self.transforms(data)

        image, lm = data["image"], data["lm"]
        assert image.shape == lm.shape and image.dim() == 4
        ...
        return {"case_id": row.get("case_id"), "ok": True, "shape": list(image.shape)}
