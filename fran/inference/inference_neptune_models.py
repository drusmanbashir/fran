...
# %%
    ...
    masks_valid = [filename for filename in mask_files if  drop_digit_suffix(filename.name) in valid_list]
    imgs_valid =  [proj_defaults.raw_data_folder/"images"/mask_file.name for mask_file in masks_valid]
    imgs_test =  [filename for filename in img_files if  drop_digit_suffix(filename.name) in test_list]
