...
from fran.inference.base_old import (
    BaseInferer,
    get_patch_spacing,
    list_to_chunks,
    load_params,
...
# %%
    ...
    # p = En.W.predict()
        En.W.create_postprocess_transforms(En.W.ds.transform)
        pred = En.W.postprocess_transforms(batch)
        if En.save_localiser == True:
            En.W.save_pred(pred)
        pred = Sel(pred)
        pred = B(pred)
        bb = pred["bounding_box"]
