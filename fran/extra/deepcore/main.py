...
def main():
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="the number of training epochs to be preformed between two test epochs; a value of 0 means no test will be run (default: 1)",
    )
    parser.add_argument(
        "--test_fraction",
        "-tf",
    ...
    if args.resume != "":
        ...
        try:
            ...
            try:
                ...
                assert "indices" in checkpoint["subset"].keys()
                print(
                    "=> The checkpoint only contains the subset, training will start from the begining"
                )
                start_exp = checkpoint["exp"]
                start_epoch = 0
            except AssertionError:
    ...
    for exp in range(start_exp, args.num_exp):
        if args.save_path != "":
            checkpoint_name = (
                "{dst}_{net}_{mtd}_exp{exp}_epoch{epc}_{dat}_{fr}_".format(
                    dst=args.dataset,
                    net=args.model,
                    mtd=args.selection,
                    dat=datetime.now(),
        ...
        if "subset" in checkpoint.keys():
            ...
            method = met.__dict__[args.selection](
            ...
            dst_train.transform = transforms.Compose(
                [
                    transforms.RandomCrop(
                        args.im_size, padding=4, padding_mode="reflect"
                    ),
        ...
        for model in models:
            ...
            network = nets.__dict__[model](channel, num_classes, im_size).to(
                args.device
            )
            ...
            if args.save_path != "" and args.resume == "":
                save_checkpoint(
                    ...
                    os.path.join(
                        args.save_path,
                        checkpoint_name
                        + ("" if model == args.model else model + "_")
                        + "unknown.ckpt",
                    ),
                    0,
                    0.0,
            ...
            print(
                "| Best accuracy: ",
                best_prec1,
                ", on model " + model if len(models) > 1 else "",
                end="\n\n",
            )
            start_epoch = 0
            checkpoint = {}
