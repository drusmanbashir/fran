...
### Introduction
To advance the research of coreset selection in deep learning, we contribute a code library named **DeepCore**, an extensive and extendable code library, for coreset selection in deep learning, reproducing dozens of popular and advanced coreset selection.met and enabling a fair comparison of different.met in the same experimental settings. **DeepCore** is highly modular, allowing to add new architectures, datasets,.met and learning scenarios easily. It is built on PyTorch.   

### Coreset.met
We list the.met in DeepCore according to the categories in our original paper, they are 1) geometry based.met Contextual Diversity (CD), Herding  and k-Center Greedy; 2) uncertainty scores; 3) error based.met Forgetting  and GraNd score ; 4) decision boundary based.met Cal  and DeepFool ; 5) gradient matching based.met Craig  and GradMatch ; 6) bilevel optimiza- tion.met Glister ; and 7) Submodularity based.met (GC) and Facility Location (FL) functions. we also have Random selection as the baseline.
