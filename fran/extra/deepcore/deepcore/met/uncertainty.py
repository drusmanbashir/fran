...
class Uncertainty(EarlyTrain):
    def __init__(
        ...
        random_seed=None,
        epochs=200,
        selection_method="LeastConfidence",
        specific_model=None,
        balance=False,
        **kwargs,
    ):
        ...
        with torch.no_grad():
            ...
            for i, (input, _) in enumerate(train_loader):
                ...
                if self.selection_method == "LeastConfidence":
                    scores = np.append(
                        scores,
                        self.model(input.to(self.args.device))
                        .max(axis=1)
                        .values.cpu()
                        .numpy(),
                    )
                    ...
                    preds = (
                        torch.nn.functional.softmax(
                            self.model(input.to(self.args.device)), dim=1
                        )
                        .cpu()
                        .numpy()
                    )
                    ...
                    scores = np.append(
                        ...
                        (
                            max_preds
                            - preds[
                                torch.ones(preds.shape[0], dtype=bool), preds_sub_argmax
                            ]
                        )
                        .cpu()
                        .numpy(),
