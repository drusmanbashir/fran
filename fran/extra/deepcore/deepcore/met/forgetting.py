...
class Forgetting(EarlyTrain):
    ...
    def after_loss(self, outputs, loss, targets, batch_inds, epoch):
        with torch.no_grad():
            ...
            self.forgetting_events[
                torch.tensor(batch_inds)[(self.last_acc[batch_inds] - cur_acc) > 0.01]
            ] += 1.0
        ...
        if batch_idx % self.args.print_freq == 0:
            print(
                ...
                % (
                    ...
                    loss.item(),
                    100.0 * self.correct.item() / self.total,
                )
            )
    ...
    def before_run(self):
        ...
        self.forgetting_events = torch.zeros(self.n_train, requires_grad=False).to(
            self.args.device
        )
        self.last_acc = torch.zeros(self.n_train, requires_grad=False).to(
            self.args.device
        )
    ...
    def select(self, **kwargs):
        ...
        if not self.balance:
            top_examples = self.train_indx[
                np.argsort(self.forgetting_events.cpu().numpy())
            ][::-1][: self.coreset_size]
        else:
            ...
            for c in range(self.num_classes):
                ...
                top_examples = np.append(
                    top_examples,
                    c_indx[
                        np.argsort(self.forgetting_events[c_indx].cpu().numpy())[::-1][
                            :budget
                        ]
                    ],
                )
