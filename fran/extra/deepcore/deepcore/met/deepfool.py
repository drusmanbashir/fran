...
class DeepFool(EarlyTrain):
    ...
    def finish_run(self):
        ...
        for i, (inputs, targets) in enumerate(batch_loader):
            ...
            r[
                (i * self.args.selection_batch) : (
                    i * self.args.selection_batch + targets.shape[0]
                )
            ] = self.deep_fool(inputs)
    ...
    def deep_fool(self, inputs):
        ...
        r_tot = np.zeros(
            [sample_size, inputs.shape[1] * inputs.shape[2] * inputs.shape[3]]
        )
        ...
        for i in range(self.max_iter):
            ...
            w_k_norm[first_preds, boolean_mask[boolean_mask]] = (
                1.0  # Set w_k_norm for preds positions to 1. to avoid division by zero.
            )

            l_all = (
                f_all - f_all[boolean_mask[boolean_mask], first_preds].unsqueeze(1)
            ).detach().abs() / w_k_norm.T
            l_all[boolean_mask[boolean_mask], first_preds] = (
                np.inf
            )  # Set l_k for preds positions to inf, as the argmin for each
            # row will be calculated soon.

            l_hat = l_all.argmin(dim=1)
            r_i = (
                l_all[boolean_mask[boolean_mask], l_hat].unsqueeze(1)
                / w_k_norm[l_hat, boolean_mask[boolean_mask]].T.unsqueeze(1)
                * w_k[l_hat, boolean_mask[boolean_mask]]
            )
