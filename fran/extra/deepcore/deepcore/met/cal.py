...
from .met_utils.cossim import cossim_pair_np
from .met_utils.euclidean import euclidean_dist_pair_np
...
class Cal(EarlyTrain):
    def __init__(
        ...
        **kwargs,
        ...
        if metric == "euclidean":
            ...
            self.metric = lambda a, b: -1.0 * cossim_pair_np(a, b)
    ...
    def find_knn(self):
        ...
        model = nets.__dict__[self.pretrain_model](
        ...
        if self.args.im_size[0] != 224 or self.args.im_size[1] != 224:
            ...
            dst_train.transform = transforms.Compose(
                [dst_train.transform, transforms.Resize(224)]
            )
        else:
        ...
        if self.balance:
            ...
            for c in range(self.args.num_classes):
                ...
                batch_loader = torch.utils.data.DataLoader(
                    torch.utils.data.Subset(dst_train, class_index),
                ...
                for i, (aa, _) in enumerate(batch_loader):
                    if i % self.args.print_freq == 0:
                        print(
                            "| Caculating embeddings for batch [%3d/%3d]"
                            % (i + 1, batch_num)
                        )
                    model(aa.to(self.args.device))
                    embdeddings.append(
                        model.embedding_recorder.embedding.flatten(1).cpu().numpy()
                    )

                embdeddings = np.concatenate(embdeddings, axis=0)

                knn.append(
                    np.argsort(self.metric(embdeddings), axis=1)[
                        :, 1 : (self.neighbors + 1)
                    ]
                )
            return knn
            ...
            return np.argsort(self.metric(embdeddings), axis=1)[
                :, 1 : (self.neighbors + 1)
            ]
    ...
    def calc_kl(self, knn, index=None):
        ...
        for i, (inputs, _) in enumerate(batch_loader):
            probs[
                i * self.args.selection_batch : (i + 1) * self.args.selection_batch
            ] = (
                torch.nn.functional.softmax(
                    self.model(inputs.to(self.args.device)), dim=1
                )
                .detach()
                .cpu()
            )
        ...
        for i in range(0, sample_num, self.args.selection_batch):
            if i % self.args.print_freq == 0:
                print(
                    "| Caculating KL-divergence for batch [%3d/%3d]"
                    % (i // self.args.selection_batch + 1, batch_num)
                )
            aa = np.expand_dims(probs[i : (i + self.args.selection_batch)], 1).repeat(
                self.neighbors, 1
            )
                ...
                selection_result = np.append(
                    ...
                    class_index[
                        np.argsort(
                            ...
                            scores[-1]
                        )[::1][: round(self.fraction * len(class_index))]
                    ],
                )
        else:
            selection_result = np.argsort(self.calc_kl(self.knn))[::1][
                : self.coreset_size
            ]
