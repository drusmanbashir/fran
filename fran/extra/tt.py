...
def softmax_derivative(softmax_output):
    ...
    softmax_output = softmax_output.view(
        B, C, -1
    )  # Reshape to [B, C, H*W] for easier manipulation

    jacobian = torch.zeros(
        B, C, C, H * W, device=softmax_output.device
    )  # Jacobian [B, C, C, H*W]
...
if __name__ == "__main__":
    ...
    input_data = torch.randn(
        5, 3, 32, 32, requires_grad=True
    )  # Batch of 5, 3-class predictions for 32x32 images

    # Randomly generated one-hot encoded target data (B=5, H=32, W=32, C=3)
    target_data = torch.randint(0, 3, (5, 32, 32))
    target_onehot = (
        F.one_hot(target_data, num_classes=3).permute(0, 3, 1, 2).float()
