...
if __name__ == "__main__":
    ...
    D = DCMCaseToSITK(
        dataset_name="lits",
        case_folder=input_fldr,
        output_folder=output_fldr,
        max_series_per_case=1,
    )
    D.process()
    ...
    left_content.append(
        Paragraph(f"Volume: {liver_data['volume']:.1f} cm³", styles["Normal"])
    )
    left_content.append(Spacer(1, 5))
    left_content.append(
        Paragraph(f"Diameter: {liver_data['diameter']:.1f} mm", styles["Normal"])
    )
    ...
    if not tumour_data.empty:
        ...
        elements.append(
            Paragraph(f"Total number of lesions: {num_lesions}", styles["Normal"])
        )
        ...
        for idx, row in tumour_data.iterrows():
            tumour_table_data.append(
                [str(idx + 1), f"{row['volume']:.1f}", f"{row['diameter']:.1f}"]
            )

        tumour_table = Table(tumour_table_data)
        tumour_table.setStyle(
            TableStyle(
                [
