...
def convert_to_bytes(img_fn: str) -> bytes:
    ...
    spacing = torch.as_tensor(meta.get("spacing", [1.0, 1.0, 1.0]), dtype=torch.float64)
    origin = torch.as_tensor(meta.get("origin", [0.0, 0.0, 0.0]), dtype=torch.float64)
    direction = torch.as_tensor(meta.get("affine", np.eye(4)), dtype=torch.float64)
