...
class ConfigMaker:
    ...
    def add_out_channels(self):
        if "src_dest_labels" in self.config["plan"].keys():
            out_ch = out_channels_from_dict_or_cell(
                self.config["plan"]["src_dest_labels"]
            )
        else:
    ...
    def add_patch_size(self):
        if not "patch_size" in self.config["dataset_params"]:
            self.config["dataset_params"]["patch_size"] = make_patch_size(
                self.config["dataset_params"]["patch_dim0"],
                self.config["dataset_params"]["patch_dim1"],
            )
