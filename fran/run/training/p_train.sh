...
exec "$PYTHON_BIN" "$BLOCK_SUSPEND" "$SUSPEND_ONLY" "$TRAIN_PY" --project "$PROJECT" --plan-num "$PLAN_NUM" --fold "$FOLD" --epochs "$EPOCHS" --devices "$DEVICES" --wandb "$WANDB" --val-every-n-epochs "$VAL_EVERY_N_EPOCHS" "$RUN_NAME_FLAG"
