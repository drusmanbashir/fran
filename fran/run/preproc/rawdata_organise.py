...
class PrepRawData:
    ...
    def verify_img_mask_pairs(self):
        ...
        for mask_fn in self.mask_src_files:
            corresp_img_fn = [
                img_fn
                for img_fn in self.img_src_files
                if img_fn.name.replace(self.mask_id, self.img_id) == mask_fn.name
            ]
            missing_imgs = []
            assert len(corresp_img_fn) < 2, (
                "Multiple matching image files {}. Please reconcile and start again.".format(
                    corresp_img_fn
                )
            )
        ...
        if len(missing_imgs) > 0:
            ...
            print(
                "Some mask file names couldn't be matched. Names of those masks are stored {}/missing_files.txt".format(
                    error_fn
                )
            )
            save_list(error_fn)
    ...
    def create_output_filenames(self):
        ...
        for fn_pair in self.src_file_pairs:
            dest_pair = [
                fn.str_replace(id, self.dataset_name)
                for fn, id in zip(fn_pair, [self.img_id, self.mask_id])
            ]
    ...
    def rename_and_copy(self):
        args = [
            [*src_pair, *dest_pair]
            for src_pair, dest_pair in zip(self.src_file_pairs, self.dest_file_pairs)
        ]
        multiprocess_multiarg(
            copy_src_dest, args, self.num_processes, debug=self.debug, io=True
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Organises downloaded data and renames it to fran-compatible folder and filenames. Currently supports LITS and kits2 only."
    )
