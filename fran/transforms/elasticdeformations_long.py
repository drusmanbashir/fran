...
sys.path += [
    "/home/ub/Dropbox/code/nurbspy/nurbspy",
    "/home/ub/Dropbox/code/nurbspy/",
...
if __name__ == "__main__":
    ...
    fold = 0
    train_list, valid_list, _ = get_fold_case_ids(
        project_title=proj_defaults.project_title, fold=fold, json_fname=json_fname
    )
    ...
    h = m + p + 1
    U_clamped = torch.cat(
        [torch.zeros(p), torch.linspace(0, 1, h + 3 - 2 * p), torch.ones(p)]
    )
    k = n + q + 1
    V_clamped = torch.cat(
        [torch.zeros(q), torch.linspace(0, 1, k + 3 - 2 * q), torch.ones(q)]
    )
    ...
    z = torch.tensor([0.0, 0.3, 0.4, 0.0]).repeat(4, 1)
    print(z.shape)
    ...
    folder_p = proj_defaults.stage2_folder / (
        "{0}_{1}_{1}".format(patch_size[0], patch_size[1])
    )
    ...
    inds_1d = [
        0,
    ] + [int(gap * mult - 1) for mult in range(1, num_points + 1)]
    ...
    _, _, test_list = get_train_valid_test_lists_from_json(
        project_title=proj_defaults.project_title,
        fold=0,
        json_fname=proj_defaults.validation_folds_filename,
        image_folder=proj_defaults.raw_data_folder / "images",
        ext=".nii.gz",
    )
    mask_files = [
        proj_defaults.raw_data_folder
        / "lms"
        / test_file.name.replace(".npy", ".nii.gz")
        for test_file in test_list
    ]
    img_files = [
        proj_defaults.raw_data_folder / "images" / mask_file.name
    ...
    sobel_kernel = (
        torch.tensor(sobel, dtype=torch.float32)
        .unsqueeze(0)
        .expand(depth, 1, channels, 3, 3)
    )
    # %%
    ...
    cp_inds = np.array(
        ...
        + [random.randint(0, len(indices)) for x in range(14)]
        + [-1]
    )
    i = indices[cp_inds, :]
    ...
    plt.show()
    # %%
    symm = mvn(mn2, cov2=torch.tensor([[0.2, 0], [0, 0.3]]))
    torch.exp(symm).max()
    # %%
    fig, ax = plt.subplots()
    ...
    plt.quiver(
        base_mesh[0, 10:25, 25:35, 0],
        base_mesh[0, 10:25, 25:35, 1],
        base_mesh[0, 10:25, 25:35, 0],
        base_mesh[0, 10:25, 25:35, 1],
    )
    # %%
    ...
    base_mesh3d_big = F.interpolate(
        base_mesh3d.permute(0, 4, 1, 2, 3), size=(128, 128, 128), mode="trilinear"
    ).permute(0, 2, 3, 4, 1)
    # %%
    n = 0
    ...
    # %%
    grads3d_big = F.interpolate(
        grads3d.permute(0, 4, 1, 2, 3), size=(128, 128, 128), mode="trilinear"
    )
    ...
    # %%
    surf = ax.quiver(
        minimesh[:, :, 0], minimesh[:, :, 1], z, linewidth=0, antialiased=False
    )
    # %%
    ...
    grid = get_affine_grid(aa.shape, scale_ranges=[1.0, 1.0])
    g = grid + ww
    ...
    for i in range(base_mesh.shape[1]):
        for j in range(base_mesh.shape[2]):
            ...
            dists[i, j] = (
                (p_ij - p_rand) - (torch.dot(p_ij - p_rand, v_normed)) * v_normed
            ).norm()
    ...
    LUT_Bspline_x, LUT_Bspline_y, LUT_Bspline_z = [
        np.zeros((aa * 4)) for aa in tile_dims
    ]
    for j in range(4):
        for i in range(tile_dims[0]):
            LUT_Bspline_x[j * tile_dims[0] + i] = compute_basis_function(
                j, i, tile_dims[0]
            )
        for i in range(tile_dims[1]):
            LUT_Bspline_y[j * tile_dims[1] + i] = compute_basis_function(
                j, i, tile_dims[1]
            )
        for i in range(tile_dims[2]):
            LUT_Bspline_z[j * tile_dims[2] + i] = compute_basis_function(
                j, i, tile_dims[2]
            )
