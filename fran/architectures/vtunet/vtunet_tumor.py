...
from fran.architectures.vtunet.neural_network import SegmentationNetwork
...
class PatchExpand_Up(nn.Module):
    ...
    def forward(self, x):
        ...
        was_thirtytwo = x.shape[2]
        x = x.flatten(2).transpose(1, 2)
        x = self.expand(x)
        B, L, C = x.shape
        # assert L == D * H * W, "input feature has wrong size"

        x = x.view(B, was_thirtytwo, H, W, C)
...
class VTUNet(SegmentationNetwork):
    def __init__(self, pretrain_ckpt="./pretrained_ckpt/swin_tiny_patch4_window7_224_22k.pth", num_classes=3, zero_head=False, embed_dim=48, win_size=7,
        ...
        self.swin_unet = SwinTransformerSys3D(img_size=img_size,
                                            patch_size=(4, 4, 4),
                                            in_chans=1,
...
# %%
    import yaml
    import os; os.chdir('/home/ub/Dropbox/code/vtunet')
    cfg = "vt/vtunet/configs/vt_unet_small.yaml"
    with open(cfg, 'r') as f:
        yaml_cfg = yaml.load(f, Loader=yaml.FullLoader)
    yaml_cfg['MODEL']['PRETRAIN_CKPT']=None
# %%
    img_size=(64,160,160)
    x = torch.rand(1,1,*img_size)
# %%
    net= VTUNet(pretrain_ckpt=yaml_cfg.get("MODEL").get("PRETRAIN_CKPT"), num_classes=3,
