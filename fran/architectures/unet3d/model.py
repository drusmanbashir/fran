...
from fran.architectures.unet3d.buildingblocks import (
    ...
    ExtResNetBlock,
    SingleConv,
    create_attention_gates,
    create_bottlenecks,
    create_decoders,
    create_deep_segmenters,
    create_encoders,
)
from fran.architectures.unet3d.utils import get_class, number_of_features_per_level
...
class Abstract3DUNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.bottlenecks = create_bottlenecks(
            f_maps[-1],
    ...
    def forward(self, x):
        ...
        # decoder part
        seg_outputs = []
        for decoder, attention_gate, encoder_features, deep_seg in zip(
            self.decoders, self.attention_gates, encoders_features, self.deep_segs
        ):
...
def get_model(model_config):
    model_class = get_class(
        model_config["name"], modules=["architectures.unet3d.model"]
    )
...
if __name__ == "__main__":
    ...
    # %%
    # create encoder path
    model.encoders = create_encoders(
        in_channels,
        f_maps,
        basic_module,
        conv_kernel_size,
    ...
    n_bottlenecks = 0
    in_out_channels = 32
    b = create_bottlenecks(
        f_maps[-1], basic_module, layer_order, n_bottlenecks=n_bottlenecks
    )
    # %%
    # create decoder path
    model.decoders = create_decoders(
        f_maps,
        basic_module,
        conv_kernel_size,
        conv_padding,
