...
from fran.data.dataset_fastai import *
...
from fran.callback.case_recorder_fa import CaseIDRecorder_FA
...
class Trainer:
    ...
    def create_transforms(self):
        ...
        self.after_item_train = Pipeline(
            [
                ...
                intensity.ClipCenter(self.dataset_params['clip_range'],self.dataset_params['mean_fg'],self.dataset_params['std_fg']),
    ...
    def create_datasets(
        ...
        self.train_ds = ImageMaskBBoxDataset_FA(
            self.train_list,
            bboxes_fname,
            self.dataset_params["class_ratios"],
        )
        self.valid_ds = ImageMaskBBoxDataset_FA(
            self.valid_list,
            bboxes_fname,
        )
    ...
    def create_learner(self, cbs=[], distributed=False,compile=False, **kwargs):
        ...
        if (
            ...
            if not any([cc.name=='downsample_mask_for_ds' for cc in self.cbs]):
                cbs += [DownsampleMaskForDS_Fastai(self.deep_supervision_scales)]
    ...
    def fromNeptuneRun(
        ...
        **kwargs
        ...
        cbs += [
            ...
            NeptuneCallback.from_existing_run(proj_defaults=project, config_dict  = config_dict, run_name = run_name, nep_run= Nep.nep_run),
    ...
    def fromExcel(self, project, **kwargs):
        ...
        config_dict= ConfigMaker2(project,raytune=False).config
...
if __name__ == "__main__":
    ...
    configs= ConfigMaker2(project, raytune=False).config
...
# %%
    La = Trainer.fromExcel(project,cbs=cbs,bs=6)
# %%
    run_name = "LITS-627"
...
# %%
    ...
    y = y[0].detach().cpu()
# %%
    ImageMaskViewer([img[0,0],y[0,1]])
# %%
    learn.fit(n_epoch=300, lr=1e-3)
# # %%
...
n=1
[print(cbs, cbs.name) for cbs in cbs]
