...
class LoadHDF5ShardIndexd(MapTransform):
    ...
    def _cached_manifest(self, data_folder: Path) -> dict[str, Any]:
        ...
        if not isinstance(shards, list) or len(shards) == 0:
            raise ValueError(f"Invalid or empty HDF5 shard manifest: {manifest_fn}")

        case_to_shard: dict[str, str] = {}
        for shard_info in shards:
            shard_name = shard_info.get("shard")
            case_ids = shard_info.get("case_ids") or []
            if shard_name is None:
                continue
            shard_path = manifest_fn.parent / shard_name
            for case_id in case_ids:
                case_to_shard[str(case_id)] = str(shard_path)

        if len(case_to_shard) == 0:
            raise ValueError(f"No case_id -> shard mapping found in {manifest_fn}")
    ...
    def __call__(self, data):
        ...
        shard_path = manifest["case_to_shard"].get(case_id)
        if shard_path is None:
            raise KeyError(
                f"Case '{case_id}' not present in shard manifest {manifest['manifest_fn']}"
            )

        h5py = import_h5py()
        case_path = f"/cases/{case_id}"
        with h5py.File(shard_path, "r") as h5f:
            if case_path not in h5f:
                raise KeyError(f"Case path missing in shard {shard_path}: {case_path}")
            ...
            src_dims = tuple(
                int(v) for v in h5f.attrs.get("src_dims", manifest["src_dims"])
            )
...
class RandCropByFlatIndicesd(RandomizableTransform, MapTransform):
    ...
    def _sample_pool(self, fg: np.ndarray, bg: np.ndarray) -> tuple[np.ndarray, bool]:
        if fg.size == 0 and bg.size == 0:
            raise ValueError("Both fg/bg flat index arrays are empty.")

        total = max(0.0, self.pos) + max(0.0, self.neg)
        choose_fg = self.R.rand() < (self.pos / total) if total > 0 else fg.size > 0

        if choose_fg and fg.size > 0:
            return fg, True
        if (not choose_fg) and bg.size > 0:
            return bg, False
        if fg.size > 0:
            return fg, True
        return bg, False
    ...
    def __call__(self, data):
        ...
        if len(src_dims) != len(self.roi_size):
            raise ValueError(
                f"src_dims rank {len(src_dims)} != roi_size rank {len(self.roi_size)}"
            )
...
class DataManager(LightningDataModule):
    ...
    def create_transforms(self):
        ...
        Ld = LoadHDF5ShardIndexd(keys=["case_id", "data_folder"])
        ...
        L2 = LoadHDF5Cropd(
            keys=["hdf5_shard_path", "hdf5_case_path", "crop_slices"]
        )
    ...
    def create_dataset(self):
        ...
        print(
            f"[DEBUG] Example case: {example_ref}"
        )
