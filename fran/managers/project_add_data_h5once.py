...
class Project(DictToAttr):
    ...
    def vars_to_sql(self, ds_name, ds_alias, img_fn, lm_fn, test, labels=None, NNZ=False):
        ...
        cols = (
            ...
            fold,
            test,
            json.dumps([int(x) for x in labels]),
            NNZ,
        )
        return cols
    ...
    def _create_datasources_table(self):
        ...
        if not self.table_exists(tbl_name):
            self.sql_alter(
                "CREATE TABLE {} (ds,alias,case_id,image,lm,img_symlink,lm_symlink,fold INTEGER,test,labels,NNZ)".format(
                    tbl_name
                )
            )
    ...
    def add_data(self, datasources: list, test=False, multiprocess=False):
        ...
        for dataspec, test in zip(datasources, test):
            ...
            if not ds.h5_fname.exists():
                ds.process(multiprocess=multiprocess)
            ds = self.filter_existing_images(ds)
            case_info = {}
            ds_labels = []
            import h5py
            with h5py.File(ds.h5_fname, "r") as h5f:
                for img_fn, _ in ds.verified_pairs:
                    case_id = info_from_filename(img_fn.name, full_caseid=True)["case_id"]
                    labels = list(h5f[case_id].attrs["labels"]) if case_id in h5f else []
                    case_info[case_id] = {"labels": labels, "NNZ": not any(int(x) != 0 for x in labels)}
                    ds_labels.extend(labels)
            self.populate_tbl(ds, case_info)
            dici = {
                "ds": dataspec.ds,
                "alias": dataspec.alias,
                "ds_type": ds_type,
                "folder": str(fldr),
                "h5_fname": str(ds.h5_fname),
                "labels": sorted(list({int(x) for x in ds_labels})),
            }
            dicis.append(dici)
    ...
    def populate_tbl(self, ds, case_info=None):
        """
        ...
        """
        case_info = {} if case_info is None else case_info
        strs = []
        for pair in ds.verified_pairs:
            case_id = info_from_filename(pair[0].name, full_caseid=True)["case_id"]
            d = case_info.get(case_id, {})
            strs.append(self.vars_to_sql(ds.name, ds.alias, *pair, ds.test, d.get("labels"), d.get("NNZ", False)))
        with db_ops(self.db) as cur:
